...
- [ ] **T9** `db.rs` — checkpoint takeover, LTX capture loop, snapshot-on-continuity-break, clean shutdown. Port `db_test.go`, `db_internal_test.go`, `db_shutdown_test.go`. *(dep: T1, T2)* — **IN PROGRESS** (HIGH RISK / largest task; recon brief `docs/porting-briefs/T9.md`). **Sub-step 1 done 2026-05-30:** `Db::open` — WAL-mode SQLite setup (busy_timeout, wal_autocheckpoint(0), journal_mode=WAL verified, `_litestream_seq`/`_litestream_lock` tables, page-size) + meta/WAL paths, tested against real SQLite (2 tests). **Remaining sub-steps:** read-lock takeover (R-4 concurrency / dedicated thread), PERSIST_WAL (likely `unsafe` FFI), WAL→LTX capture loop, `verify()` snapshot-on-continuity-break, clean shutdown.
